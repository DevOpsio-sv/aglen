...
function buildPageSpecificSchemas(lang: LanguageCode, routeId: RouteId, routeUrl: string): object[] {
  ...
  // An Event called "seasonal travel updates", running 2026-06-01 to 2026-12-31,
  // used to be emitted here for /events and /seasonal. A seven-month visit window
  // is not an event: like the VideoObject below, it claimed a rich result out of
  // nothing the page actually has. Only the real events in events.ts are marked up.
  ...
  if (routeId === "events") {
    for (const item of sortedEvents()) {
      schemas.push({
        ...
        // A one-day event ends the day it starts — schema.org wants that said out loud.
        endDate: item.endDate ?? item.startDate,
        ...(item.admission
          ? {
              offers: {
                "@type": "Offer",
                price: item.admission === "free" ? "0" : item.admission,
                priceCurrency: "BGN",
                availability: "https://schema.org/InStock",
                url: item.ctaUrl ?? routeUrl,
              },
            }
          : {}),
        // No `performer`: neither a village fair's line-up nor a photo contest has a
        // named performer we can source. Google warns about the gap; inventing one
